...
A running index of how many tokens the world's LLMs process each day, published
at [tokensperday.com](https://tokensperday.com/). It is not a dataset release —
it is one publisher's estimate, and its whole structure is a single distinction:

- a **disclosed floor** of 300.1 trillion tokens/day, the sum of six company or
  national statements that do not overlap, and
- an **estimated total** of 360.4 trillion, the floor plus a six-channel
  triangulation over every provider that discloses nothing.

**14 charts**, in two sections:

| IDs | Section | What they are |
|---|---|---|
| `TPD-01` – `TPD-06` | Tokens Per Day — Published Visualizations | The site's own charts: the three tabs of its time series, the 24-hour cycle, the all-time cumulative curve, and the quarterly estimate band from `/estimates/` |
| `TPD-D01` – `TPD-D08` | Derived Analysis | Questions its three charts do not answer — how much of the index is evidence rather than inference, what the evidence ledger actually contains, what each company's number is worth, the hardware ceiling, the other published estimates, and two tests of what the site's own charts are made of |

`TPD-01`, `TPD-02`, `TPD-03`, `TPD-D04` and `TPD-D05` ship interactive
companions, generated by `generate_interactive_tokensperday.py` and checked by
`verify_interactive_tokensperday.mjs`. The three time-series pages replay the
site's own tooltip rule — each series at its most recent disclosure on or before
the cursor, carried forward, rows sorted by value — and add the date each
carried-forward figure was actually disclosed on, which the site leaves implicit.
The verifier checks that rule against every observation date rather than only
checking that a tooltip appears, and asserts the case the extraction documents:
hovering early November 2025 gives China 54.84, and 15 November gives 74.06. The page also carries a glossary, because
the difference between the floor and the estimate is the domain and a reader who
misses it will read a modelled number as a measured one.

### Where the data came from

tokensperday.com is a static Astro build with **no API and no data endpoint**.
Every chart it draws comes from a JavaScript object literal embedded in its
homepage, and the extraction lifted those literals character for character. That
has one consequence which governs the whole domain: the plotted values here *are*
the source. Nothing is refitted, interpolated or recomputed, and where the ledger
holds more precision than the chart does — Google plots `105.1`, the ledger holds
`105.133` — both travel and the chart's own number is the one plotted.

Three rules hold across all 14:

- **Measured and modelled stay apart.** The site flags every point as a real
  observation or not, and it is preserved rather than flattened. It matters in
  one place and matters a lot: the China line in the country tab has 27 plotted
  points and **3 measurements**.
- **The floor is six named rows, not a rollup.** It is not the latest figure per
  company — Microsoft's in-floor row is its 2024-12-30 disclosure while its later
  one is out — and overlapping rows are marked `contained` and excluded, so
  Doubao's 180T/day, the largest number in the ledger, adds nothing.
- **Estimates are bands, never points.** Every per-company figure is a
  low/mid/high, and the widths differ by two orders of magnitude between
  companies. Any table or chart here that carries a middle carries its band
  beside it.

### What the derived charts found

Three findings came out of testing the site's charts rather than reproducing
them, and each is stated on the chart that makes it:

- **The country tab is not five measurements.** The four non-China regions are
  one number — non-China tokens — split 47/18/13/22 by OpenRouter's
  billing-geography mix. Across the 12 dates where the base is large enough for
  the site's 0.1T rounding not to dominate, those implied shares hold to **0.32
  percentage points**, against a 0.27pp tolerance from rounding alone.
  `TPD-D03` shows the arithmetic rather than asserting the conclusion.
- **The cumulative chart is the daily estimate added up.** Integrating the
  plotted daily estimate reproduces the published all-time curve to **0.10%** at
  worst and 0.001% at the end. It is kept because the site publishes it and
  quotes the 92.6-quadrillion figure, and `TPD-05` draws the reconstruction
  beside it so that is visible rather than assumed.
- **The floor has closed on the estimate.** It was 6% of the estimate in October
  2024 and is 83% now — almost entirely four disclosures arriving between January
  and June 2026, not a change of method. `TPD-D01` plots the ratio, which appears
  on no published view.

The country tab also sums to the **floor**, not to the estimate: the five
regional series total 300.3T/day while the estimate stands at 360.4T. The
60T/day difference is the unattributed remainder and appears in no region.
...
│   ├── summarise_tokensperday.py  tokensperday.com extraction  →  derived tables
│   ├── generate_interactive_tokensperday.py  the three-tab chart, with its tooltip
...
Per-domain, the same three steps. Tokens Per Day, for example:

```bash
python build/summarise_tokensperday.py                       # extraction  →  tables
python build/generate_charts.py TPD-01 TPD-02 TPD-03 TPD-04 TPD-05 TPD-06 \
    TPD-D01 TPD-D02 TPD-D03 TPD-D04 TPD-D05 TPD-D06 TPD-D07 TPD-D08
python build/generate_interactive_tokensperday.py            # interactive companions
node   build/verify_interactive_tokensperday.mjs             # and check they respond
python build/build_site.py
```
...
| `tokensperday_chart_data` | `tokens_per_day_total.csv`, `tokens_per_day_country.csv`, `tokens_per_day_company.csv`, `hourly_tokens.csv`, `cumulative_tokens.csv`, `estimate_band_quarterly.csv` | `TPD-01` – `TPD-06` |
| `tokensperday_extraction/normalized` | `evidence_ledger.csv`, `reconciled_company_estimates.csv`, `channel_fan_per_company.csv`, `capacity_parameters.csv`, `geography_allocation_shares.csv`, `comparison_vs_independent_estimates.csv`, `headline_metrics.csv` | `TPD-D01` – `TPD-D08` |
...
```bash
python build/verify_source_files.py     # 96 published charts checked, 0 failing
```

checks all 96 published charts against it. A chart that names no file, that
